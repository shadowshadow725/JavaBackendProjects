...
public class Neo4jDAO {
    ...
    public Result getDistanceAll(double lng1, double lat1, double distance){
        String query = "MATCH (n: user {is_driver: true}) WHERE distance(point({latitude:toFloat('%f')," +
                " longitude:toFloat('%f')}), point({latitude:n.latitude, longitude:n.longitude}))/1000< %f " +
                "RETURN n.street, n.latitude, n.longitude, n.uid";
        query = String.format(query, lat1, lng1, distance);
        return this.session.run(query);
    }

    public Result getShortestPath(String passengerStreet, String driverStreet){
        String query = "MATCH (source:road {name: '%s'}), (target:road {name: '%s'}) CALL gds.shortestPath.dijkstra.stream(  " +
                "                    { " +
                "                        nodeProjection: 'road',  " +
                ...
                "                    costs, " +
                "                    [nodeId IN nodeIds | gds.util.asNode(nodeId).has_traffic] AS traffic " +
                "                     " +
                "                ORDER BY totalCost " +
                " " +
                " ";
